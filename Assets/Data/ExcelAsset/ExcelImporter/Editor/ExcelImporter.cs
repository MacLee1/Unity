...
public class ExcelImporter : AssetPostprocessor
{
	...
	static string ResourcdPath = "Assets/Resources/SheetData";
	...
	static object GetEntityDictionaryFromSheet(ISheet sheet, Type entityType)
	{
		...
		for (int i = 1; i <= sheet.LastRowNum; i++)
		{
			...
			if (entryCell.CellType == CellType.Numeric)
			{
				if (dictionaryType == null)
				{	
					dictionaryType = typeof(Dictionary<,>).MakeGenericType(typeof(int), entityType);
					dictionaryAddMethod = dictionaryType.GetMethod("Add", new Type[] { typeof(int), entityType });
					dictionary = Activator.CreateInstance(dictionaryType);
				}
				dictionaryAddMethod.Invoke(dictionary, new object[] { (int)entryCell.NumericCellValue, entity });				
			}
			else
			{
				if (dictionaryType == null)
				{
					dictionaryType = typeof(Dictionary<,>).MakeGenericType(typeof(string), entityType);
					dictionaryAddMethod = dictionaryType.GetMethod("Add", new Type[] { typeof(string), entityType });
					dictionary = Activator.CreateInstance(dictionaryType);
				}
				dictionaryAddMethod.Invoke(dictionary, new object[] { entryCell.StringCellValue, entity });
			}
		}
	...
	static void ImportExcel(string excelPath, ExcelAssetInfo info)
	{
		...
		if(string.IsNullOrEmpty(info.Attribute.AssetPath))
		{
			...
			assetPath = Path.Combine(ResourcdPath, assetName);
			if (!Directory.Exists(ResourcdPath))
				Directory.CreateDirectory(ResourcdPath);
		}
		else{
		...
		foreach (var assetField in assetFields)
		{
			...
			if (sheet.SheetName.IndexOf("_") == 0)
				continue;
			...
			if (fieldType.IsGenericType)
			{
				...
				{
					ImportExcel_ListOutput(asset, fieldType, assetField, sheet);
					sheetCount++;
				}
				//else if (fieldType.GetGenericTypeDefinition() == typeof(Dictionary<,>))
				//{
				//	ImportExcel_DictionaryOutput(asset, fieldType, assetField, sheet);
				//	sheetCount++;
				//}
			}
		...
		string sJson = JsonConvert.SerializeObject(asset);
		Debug.Log(asset.ToString() + " : " + sJson);
	...
	static void ImportExcel_ListOutput(UnityEngine.Object asset, Type fieldType, FieldInfo assetField, ISheet sheet)
	{
	...
	static void ImportExcel_DictionaryOutput(UnityEngine.Object asset, Type fieldType, FieldInfo assetField, ISheet sheet)
	{
		Type[] types = fieldType.GetGenericArguments();
		Type entityType = types[0];

		object entities = GetEntityDictionaryFromSheet(sheet, entityType);
		assetField.SetValue(asset, entities);
	}
}
